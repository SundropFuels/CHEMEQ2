...
class Tube:
    def __init__(self, D = 1.0, h = 0.0, Tw = 300.0, eps = 0.0):
        self.D = D
        
        self.h = h
        self.Tw = Tw
        self.eps = eps
        self.XSA = self.D**2/4.0*3.14159


class ChemEQ2Solver:

    def __init__(self, ct_phase, mode = 'isothermal', tube = Tube()):
    ...
    def _init_y(self, n = 1.0):
        ...
        for key in self.ct_phase.species_names:
            ...
            data[key][0] = self.ct_phase.X[self.ct_phase.species_index(key)] * n		#building off of mole fractions, with a basis scalable by volumetric flowrate
    ...
    def calc_yp(self):
        ...
        self.q0 = self.ct_phase.creation_rates*self.tube.XSA
        self.p0 = self.ct_phase.destruction_rates*self.tube.XSA/self.y0[:self.species_len]
    ...
    def calc_yc(self, yp):
        ...
        qp = self.ct_phase.creation_rates*self.tube.XSA
        pp = self.ct_phase.destruction_rates*self.tube.XSA/yp[:self.species_len]
    ...
    def energy_balance_adiabatic(self, y, Q=0.0):
        ...
        return (Q-1*(np.sum(self.ct_phase.net_production_rates*self.tube.XSA*self.ct_phase.partial_molar_enthalpies)))/(n*self.ct_phase.cp_mole)
